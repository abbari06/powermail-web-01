...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    (history.state.mode, history.state.plan);
    if ('user' in localStorage) {
      ...
      // if(history.state.plan!==true && !this.user.trialPlan){
      //   (this.authService.userModel.email,history.state.plan,this.user.trialPlan);
  ...
  fetchDashboardData() {
    this.dashboardService
      .getDashboardData(this.user.id, this.userAccount.id)
      .subscribe({
        next: (res) => {
          this.userData = res;
